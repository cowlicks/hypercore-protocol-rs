...
pub async fn main() -> Result<()> {
    ...
    info!("attempting connection to {address}");
    let stream = TcpStream::connect(&address).await?;
    info!("connected to {address}");
    let mut protocol = ProtocolBuilder::new(true).connect(TokioAsyncReadCompatExt::compat(stream));
    Ok(())
}
*/
...
use std::{collections::HashMap, convert::TryInto, env, fmt::Debug, sync::Arc};
use tokio::{
    ...
    T: RandomAccess + Debug + Send,
{
    ...
    let mut protocol =
        ProtocolBuilder::new(is_initiator).connect(TokioAsyncReadCompatExt::compat(stream));
